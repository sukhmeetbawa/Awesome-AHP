...
def get_weight(A, str, check_consistency):
    n = A.shape[0]
...
def calculate_ahp(A, B, n, m, criterions, alternatives, check_consistency):
    for i in range(0, n):
        for j in range(i, n):
            if i != j:
                A[j][i] = float(1/A[i][j])
    ...
    W2, error_criterion = get_weight(A, "Criterion Table", check_consistency)
    if error_criterion:
        return None, None, error_criterion

    W3 = np.zeros((n, m))
    for i in range(0, n):
        w3, error_alternative = get_weight(B[i], f"Alternative Table for Criterion {criterions[i]}", check_consistency)
...
def calculate_alternative_weights():
    ...
    if request.json is not None:
        ...
        check_consistency = request.json.get('check_consistency')

    if criteria_matrix is None:
        return jsonify({'error': 'Invalid input'})

    result_criterion_weights, result_alternative_weights, error_message = calculate_ahp(criteria_matrix, alternatives_matrix, len(criterions or []), len(alternatives or []), criterions or [], alternatives or [], check_consistency)
...
@app.route('/open_ai_api', methods=['POST'])  # Change the method to POST # type: ignore
def open_ai_api():
    criterias = None
    alternatives = None
    usecase = None
